...
pcl::PointCloud< PointCloudMapping::PointT >::Ptr PointCloudMapping::generatePointCloud(KeyFrame* kf, cv::Mat& color, cv::Mat& depth, PointCloud::Ptr &tmps)
{
    PointCloud::Ptr tmp( new PointCloud() );
    PointCloud::Ptr tmppre( new PointCloud() );
    // point cloud is null ptr
    ...
    while(1)
    {
...

        {
            ...
            for (size_t i=lastKeyframeSize; i<N ; i++) {
                ...
                PointCloud::Ptr tmps;
                PointCloud::Ptr surf_p = generatePointCloud(keyframes[i], colorImgs[i], depthImgs[i],tmps);
