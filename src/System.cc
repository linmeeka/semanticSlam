...
cv::Mat System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const cv::Mat &mask,
                        const cv::Mat &maskColor, const std::vector<cv::Rect> &ROIs, const std::vector<int> &ClassIdRes, 
                        const double &timestamp, cv::Mat &imRGBOut, cv::Mat &imDOut, cv::Mat &maskOut)
{
    ...
    cv::Mat Tcw = mpTracker->GrabImageRGBD(im,depthmap,mask,maskColor, ROIs, ClassIdRes, timestamp,imRGBOut,imDOut,maskOut);
...
// cv::Mat System::TrackMonocular(const cv::Mat &im, const cv::Mat &mask, const cv::Mat &maskColor, const double &timestamp)
// {
//     if(mSensor!=MONOCULAR)
//     {
//         cerr << "ERROR: you called TrackMonocular but input sensor was not set to Monocular." << endl;
...
//     cv::Mat Tcw = mpTracker->GrabImageMonocular(im,mask,maskColor,timestamp);
