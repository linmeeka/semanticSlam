...
cv::Mat System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const cv::Mat &mask,
                        const cv::Mat &maskColor, std::vector<std::shared_ptr<SegData>> &segDatas, 
                        const double &timestamp, cv::Mat &imRGBOut, cv::Mat &imDOut, cv::Mat &maskOut)
{
    ...
    cv::Mat Tcw = mpTracker->GrabImageRGBD(im,depthmap,mask,maskColor, segDatas, timestamp,imRGBOut,imDOut,maskOut);
