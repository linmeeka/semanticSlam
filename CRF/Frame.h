...
public:
    ...
    bool MovingCheckByPolar();

    bool MovingCheckBySemantic();
...
public:
    ...
    std::vector<cv::KeyPoint> erasedmKeysT;
...
private:
    ...
    //Included by Berta
    void removeRow(cv::Mat &matIn, int row);

    void drawMask(const cv::Mat &mask, const cv::Mat &img, std::string &name);
